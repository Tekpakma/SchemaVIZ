...
export function getActiveDocument(state: CanvasState): CanvasDocumentState {
      ...
      immer((set) => ({
        ...
        actions: {
          ...
          moveSelectedNodes: ({ deltaX, deltaY }) =>
            set(
              (state) => {
                const document = getActiveDocument(state)
                const selectedRootIds = document.selectedNodeIds.filter(
                  (id) => {
                    const node = document.nodesById[id]
                    if (!node?.parentGroupId) return Boolean(node)

                    return !document.selectedNodeIds.includes(
                      node.parentGroupId,
                    )
                  },
                )
        ...
        tabActions: {
          ...
          switchTab: (tabId) =>
            set(
              (state) => {
                if (!state.tabsById[tabId] || state.activeTabId === tabId)
                  return
...
export function CanvasStoreProvider({
  ...
  const [store] = useState(() => {
    ...
    if (initialTab) {
      ...
      state.tabsById[initialTab.id] = initialTab
      state.tabOrder = [initialTab.id]
      state.activeTabId = initialTab.id
    } else if (initialGraph) {
      const state = newStore.getState()
      resetDocumentGraph(getActiveDocument(state), initialGraph)
    }
    return newStore
  })
...
export function useCanvasSnapshotGetters() {
  const store = useCanvasStoreInstance()

  return useMemo(
    () => ({
      getCanvasViewportSnapshot: () =>
        getActiveDocument(store.getState()).viewport,
      getCanvasNodesSnapshot: () =>
        getActiveDocument(store.getState()).nodesById,
      getCanvasNodeIdsSnapshot: () =>
        getActiveDocument(store.getState()).nodeOrder,
      getActiveCanvasTabIdSnapshot: () => store.getState().activeTabId,
      getCanvasLayoutSnapshot: (tabId?: CanvasTabId): CanvasLayoutInput => {
        const state = store.getState()
        const document =
          getTargetDocument(state, tabId) ?? getActiveDocument(state)
        return {
      ...
      getCanvasExportSnapshot: (tabId?: CanvasTabId): CanvasExportSnapshot => {
        const state = store.getState()
        const document =
          getTargetDocument(state, tabId) ?? getActiveDocument(state)
        return {
      ...
      getCanvasActionsSnapshot: () => store.getState().actions,
      getCanvasTabActionsSnapshot: () => store.getState().tabActions,
    }),
    [store],
  )
}
...
export const useCanvasActions = () =>
  useCanvasStoreContext((state) => state.actions)
export const useCanvasTabActions = () =>
  useCanvasStoreContext((state) => state.tabActions)
...
export const useActiveCanvasTabId = () =>
  useCanvasStoreContext((state) => state.activeTabId)

export const useCanvasNodes = () =>
  useCanvasStoreContext((state) => getActiveDocument(state).nodesById)
export const useCanvasEdges = () =>
  useCanvasStoreContext((state) => getActiveDocument(state).edgesById)
export const useCanvasNode: (id: NodeId) => CanvasNode | undefined = (id) =>
  useCanvasStoreContext((state) => getActiveDocument(state).nodesById[id])
export const useCanvasNodeWidth = (id: NodeId) =>
  useCanvasStoreContext(
    (state) => getActiveDocument(state).nodesById[id]?.width,
  )

export const useCanvasNodeIds = () =>
  useCanvasStoreContext(
    useShallow((state) => getActiveDocument(state).nodeOrder),
  )
export const useCanvasEdgeIds = () =>
  useCanvasStoreContext(
    useShallow((state) => getActiveDocument(state).edgeOrder),
  )
...
export const useSelectedNodeId = () =>
  useCanvasStoreContext((state) => getActiveDocument(state).selectedNodeId)
...
export const useIsMarqueeSelecting = () =>
  useCanvasStoreContext((state) => state.isMarqueeSelecting)
export const useCanvasEditingNodeId = () =>
  useCanvasStoreContext((state) => state.editingNodeId)
export const useCanvasIsStageMounted = () =>
  useCanvasStoreContext((state) => state.isStageMounted)
export const useCanvasStageSizeValue = () =>
  useCanvasStoreContext((state) => state.stageSize)
export const useCanvasViewport = () =>
  useCanvasStoreContext((state) => getActiveDocument(state).viewport)
export const useCanvasFlowDirection = () =>
  useCanvasStoreContext((state) => getActiveDocument(state).flowDirection)
export const useCanvasRoutingAuthority = () =>
  useCanvasStoreContext((state) => getActiveDocument(state).routingAuthority)
export const useCanvasLayoutOptions = () =>
  useCanvasStoreContext((state) => getActiveDocument(state).layoutOptions)
export const useShowResolvedReferences = () =>
  useCanvasStoreContext((state) => state.showResolvedReferences)
