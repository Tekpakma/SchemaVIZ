...
VERSION_FILE = ROOT / "django_schema_viz" / "__about__.py"
VERSION_PATTERN = re.compile(
    r'^(?P<prefix>\s*__version__\s*=\s*")(?P<version>\d+\.\d+\.\d+)(?P<suffix>".*)$',
    re.MULTILINE,
)
BACKEND_TAG_PATTERN = re.compile(r"^backend-v(?P<version>\d+\.\d+\.\d+)$")
...
def latest_backend_tag_version() -> str | None:
    versions: list[str] = []
    for tag_name in output(["git", "tag", "--list", "backend-v*"]).splitlines():
        match = BACKEND_TAG_PATTERN.match(tag_name.strip())
        if match:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bump the backend version, commit it, and create the matching backend tag."
    )
...
def main() -> int:
    ...
    latest_tag_version = latest_backend_tag_version()
    base_version = max(
    ...
    tag_name = f"backend-v{next_version}"
    ...
    if latest_tag_version and version_key(latest_tag_version) > version_key(current_version):
        print(f"Latest backend tag version: {latest_tag_version}")
    ...
    run(
        ["git", "commit", "-m", f"Release backend {next_version}"],
        dry_run=args.dry_run,
    )
    run(
        ["git", "tag", "-a", tag_name, "-m", f"Release backend {next_version}"],
        dry_run=args.dry_run,
    )
